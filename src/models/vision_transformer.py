...
class VisionTransformerPredictor(nn.Module):
    ...
    def forward(self, x, masks_x, masks):
        ...
        pos_embs = self.predictor_pos_embed.repeat(B, 1, 1)  # [B, N, D]
        pos_embs = apply_masks(pos_embs, masks)  # [B*len(masks), N_tgt, D]
        pos_embs = repeat_interleave_batch(
            pos_embs, B, repeat=len(masks_x)
        )  # [B*len(masks)*len(masks_x), N_tgt, D]
        # --
        ...
        x = torch.cat(
            [x, pred_tokens], dim=1
        )  # [B*len(masks)*len(masks_x), N_ctxt + N_tgt, D]
